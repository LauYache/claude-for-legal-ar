...
## Step 4: Confirm the consent rules for the state of incorporation / jurisdiction

Check the state of incorporation or country in `~/.claude/plugins/config/claude-for-legal/corporate-legal/CLAUDE.md`. Research the written-consent/resolution requirements before drafting:

- **US (Delaware or other states):**
  - Is unanimity required for a board written consent? (Standard DGCL § 141(f) requires unanimity).
  - What form of signature is valid (wet ink, electronic)?
  - Does the charter or bylaws override any default rule?
- **Argentina:**
  - Board resolutions without meetings (*resoluciones sin reunión* or *por escrito*) are permitted under CCyCN Art. 158 unless prohibited by the bylaws (*estatuto*). 
  - All directors must participate and sign the resolution document.
  - **Director conflict of interest:** Under Art. 272 LGS, any director with an interest opposite to the company in the matter being approved must disclose it and **abstain from participating and voting** on the resolution. Conflicted directors should be listed but must not sign the resolution for that specific item.
  - **Minute transcription:** The written resolution must be transcribed into the official, rubricated board minutes book (*Libro de Actas de Directorio*) and signed by the president and participating directors.

Cite the controlling statute section and any bylaws/statuto provisions. Verify currency. Flag uncertainty for attorney verification. Add a short "State-law/Jurisdiction notice" block to the output.
