# Claude for Legal - Argentina (Adaptación Local y Versión Agnóstica)
...
Además de mantener la estructura original compatible con **Claude Code** y **Claude Desktop/Cowork** adaptada a la legislación argentina, se incluye una **versión agnóstica de prompts e instructivos de sistema** en la carpeta [agnostic-legal-ar/](./agnostic-legal-ar) para que puedas utilizar esta inteligencia legal en cualquier otro cliente o modelo (como OpenCode, Aider, ChatGPT, Claude.ai, etc.).

![Claude Code Argentina en Acción](./references/images/claude_code_screenshot.png)

> **¿Eres nuevo por aquí?** Comienza con [QUICKSTART.md](QUICKSTART.md) — se instala en 60 segundos. Este README es la referencia completa.
...
/plugin install privacy-legal@claude-for-legal-ar
/plugin install corporate-legal@claude-for-legal-ar

# Restart Claude Code, then run setup for each plugin you installed.
# This writes your practice profile to ~/.claude/plugins/config/claude-for-legal-ar/<plugin>/CLAUDE.md
...
| **Practice profile** | Plain-English `CLAUDE.md` describing your playbook, escalation rules, and house style. Every skill reads from it. | `~/.claude/plugins/config/claude-for-legal-ar/<plugin>/CLAUDE.md` |
...
- **Edit the practice profile.** Your profile lives at `~/.claude/plugins/config/claude-for-legal-ar/<plugin>/CLAUDE.md`. Edit it directly for small fixes — a wrong escalation threshold, a new integration, a policy update. It survives plugin updates.
