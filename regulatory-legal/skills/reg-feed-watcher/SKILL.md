...
Agency/Regulator slug/key reference for common regulators:
| Regulator | API slug / Feed URL |
|---|---|
| FTC (US) | federal-trade-commission |
| SEC (US) | securities-and-exchange-commission |
| BORA (AR) | boletinoficial.gob.ar |
| InfoLEG (AR) | servicios.infoleg.gob.ar/infolegInternet/rss/anexos-boletinoficial.xml |
...
- **CourtListener / Trellis MCP:** Same.

De-duplicate across tiers. Prefer the richest source for the enriched output.

**No silent supplement.** If the feed pull returns few or no results for a regulator in the watchlist, report what was found and stop. Do NOT fill the gap from web search or model knowledge without asking. Say: "The feed check returned [N] items. Coverage appears thin for [regulator]. Options: (1) broaden the date window, (2) try web search — results will be tagged `[web search — verify]`, or (3) stop here. Which would you like?"

**Source attribution.** Tag every citation and regulatory item with where it came from: `[Federal Register]`, `[BORA]`, `[InfoLEG]`, `[AFIP RSS]`, `[BCRA]`, `[CourtListener]`, or the specific MCP/URL; `[web search — verify]` for web search; `[model knowledge — verify]` for training data. Never strip or collapse the tags.

**Secondary sources.** Tag any item pulled from summaries (e.g. Errepar, El Dial, Lexology, JD Supra) as `[secondary source]` in addition to the feed-name tag — e.g. `[Errepar] [secondary source]`. In the digest, add: "→ Trace to primary: [link to official site or BORA]." Do not classify a secondary source as "Always material" until the primary source is verified.
